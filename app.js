...
/* ===========================================================
   ...
   =========================================================== */
/* Honorific cleaner: user typed "Ramesh bhai" + template adds "bhaiya"
   used to give "Ramesh bhai bhaiya". Strip trailing honorific; each template
   adds the correct one per language (Hindi=bhai/ji, Bhojpuri=bhaiya). */
...
function generateMessages({name, amount, relation, language, tone, note}){
  ...
  const SPECIAL_TONES = {
    'Savage Safe': {
      ...
      Bhojpuri:[
        ...
        `${n} bhaiya, reminder bhi thak gail ba 😄 ${amt} aaj bhej da.`,
    ...
    'Mummy Style': {
      ...
      Bhojpuri:[`${n} beta, ${amt} wapas kar da. Achha bachcha hisaab time pe clear karela 😄`, `${n}, maai kahat badi — ${amt} aaj bhej da beta 🙏`, `${n} beta, pyaar alag ba, hisaab alag ba. ${amt} clear kar da.`],
    ...
    'Sharma Ji': {
      ...
      Bhojpuri:[`Sharma ji ke beta same day ${amt} wapas kar dehlas 😄 ${n} bhaiya, aap bhi kar da.`, `${n} bhaiya, Sharma ji benchmark set kar dele baa — ${amt} aaj clear kar da 😄`, `${n}, comparison na ba... bas ${amt} yaad dilaile bani 😄`],
  ...
  const FUNNY = {
    ...
    Bhojpuri:[
      ...
      (n,a)=>`${n} bhaiya, ${a} ke GPS on ba — "Still at ${n}'s location" show hot ba \u{1F602} Transfer kar da!`,
      (n,a)=>`${n} bhaiya, ${a} itna time se baaki ba ki okar dadi aa gail \u{1F602} Ab aur mat roka!`,
      (n,a)=>`${n} bhaiya, hamaar ${a} tohar ghar ke permanent resident ban gaili ba \u{1F605} kiraya bhi maangat ba. Bhej da!`,
      (n,a)=>`${n} bhaiya, IRCTC ke waiting confirm ho jaala, train time pe aawela — tohar ${a} kabhi na aail \u{1F602}`,
      (n,a)=>`${n} bhaiya, Amazon same day delivery deta ba — tohar ${a} kab aai? \u{1F602}`,
      (n,a)=>`${n} bhaiya, "Dosti alag ba, hisaab alag ba" — ${a} aaj clear kar da \u{1F604}`,
      (n,a)=>`${n} bhaiya, maai kaheli "Waqt pe paisa dena achha kaam ba" \u{1F604} Maai ke baat maana — ${a} aaj!`,
      (n,a)=>`${n} bhaiya, ${a} ke documentary banat ba — "Wo Paisa Jo Kabhi Na Aail" \u{1F3AC} Sequel mat bana!`,
      (n,a)=>`${n} bhaiya, tuhin kahle rahla — "Bhai, kahiha ta kar dem." \u{1F604} Ta ${a} aaj bhej da!`,
      (n,a)=>`${n} bhaiya, Diwali ba — purana hisaab clear kar da \u{1FA94} Lakshmi maiya khush hoihain!`,
      (n,a)=>`${n} bhaiya, ${a} emotional ho gaili ba \u{1F622} "Kab aaun ghar?" kahat ba — aaj bhej ke khush kar da!`,
    ],
    Hindi:[
...
function safeAlternative(name, amount){
  ...
  return `${(stripHonorific(name) || 'Bhai')}, payment kaafi din se pending hai${amtText}. Kripya aaj clear kar do. Dosti apni jagah, hisaab apni jagah 🙏`;
}
...
function generateGenericReminderMessages({name, language, tone, note}){
  ...
  const bank = {
    Friendly: {
      ...
      Bhojpuri: [
        `${n} bhaiya, payment abhi baaki ba. Aaj bhej diha ta bahut help hoi ${e('🙏')}${nt}`,
        `${n} bhaiya, ek chhoti reminder — hisaab abhi pending ba. Aaj clear kar da ${e('😊')}${nt}`,
        `${n} ji, payment ke yaad dilaile bani. Suvidha ho ta aaj bhej diha ${e('🙏')}${nt}`
      ],
      English: [
    ...
    Polite: {
      ...
      Bhojpuri: [
        `Pranam ${n} ji, payment abhi baaki ba. Kripya aaj bhej diha ${e('🙏')}${nt}`,
        `${n} bhaiya, vinamra nivedan ba — pending payment aaj clear kar diha ${e('🙏')}${nt}`,
        `${n} ji, hisaab ke chhoti yaad ba. Suvidha ho ta aaj payment bhej diha ${e('🙏')}${nt}`
      ],
      English: [
    ...
    Funny: {
      ...
      Bhojpuri: [
        ...
        `${n} bhaiya, pending payment suspense ban gail ba ${e('😄')} Aaj ending kar da.${nt}`,
        `${n} ji, payment kahe ta “ghar jaana ba” ${e('😂')} Aaj bhej diha.${nt}`
      ],
      English: [
    ...
    Strong: {
      ...
      Bhojpuri: [
        `${n} bhaiya, payment bahut din se baaki ba. Aaj tak clear kar diha.${nt}`,
        `Final reminder ${n} bhaiya — pending payment aaj clear karal jaruri ba.${nt}`,
        `${n} ji, payment overdue ba. Kripya aaj hi bhej diha.${nt}`
      ],
      English: [
...
const BB_CARD_THEMES = [
  ...
  {id:'p5', chip:'\uD83D\uDCAC Chat Card', head:'\uD83E\uDE99 BaatBanao Reminder', stamp:'', art:''},
...
async function shareCardImage(taId){
  ...
  try{
    ...
    wrap.style.cssText = 'position:fixed;left:-9999px;top:0;width:720px;background:#FFF9E8;padding:30px;box-sizing:border-box;';
    clone.style.minHeight = '520px';
    clone.style.padding = '32px';
    const exportName = clone.querySelector('.rc-name'); if(exportName) exportName.style.fontSize = '34px';
    const exportAmt = clone.querySelector('.rc-amt'); if(exportAmt) exportAmt.style.fontSize = '58px';
    const exportMsg = clone.querySelector('.rc-msg-display'); if(exportMsg){ exportMsg.style.fontSize = '25px'; exportMsg.style.lineHeight = '1.55'; exportMsg.style.padding = '26px 30px'; }
    const exportMode = clone.querySelector('.rc-mode'); if(exportMode) exportMode.style.fontSize = '18px';
    const exportFoot = clone.querySelector('.rc-foot'); if(exportFoot) exportFoot.style.fontSize = '15px';
...
function generateBusinessReplies({context, language, tone, relation}){
  ...
  const extraBank = {
    Payment: {
      ...
      Bhojpuri: [`Pranam ${rel} ji, payment ke follow-up ba. Kripya status bata diha ya payment clear kar diha.`, `${rel} ji, pending payment ke chhoti yaad ba. Aaj update mil jaai ta help hoi.`, `Payment status confirm kar diha. Agar ho gail ba ta screenshot/reference bhej diha.`],
    ...
    Delivery: {
      ...
      Bhojpuri: [`Pranam ${rel} ji, order/delivery update: process chal rahat ba. Jaldi update deb.`, `${rel} ji, delivery ke update — order process mein ba. Agila update jaldi mil jaai.`, `Aapke order note ho gail ba. Timeline confirm karke batayib.`],
    ...
    Complaint: {
      ...
      Bhojpuri: [`Khed ba ${rel} ji, asuvidha ke liye maafi. Complaint note kar lele bani, priority par check karat bani.`, `${rel} ji, issue batave ke dhanyavaad. Verify karke solution/update deb.`, `Maafi chaht bani. Thoda time diha, issue check karke best solution deb.`],
...
function generateMastiMessages({name, language, mood}){
  const n = stripHonorific(name) || 'Dost';
  const lang = language || 'Hinglish';
  const bank = {
    Friendly:{
      ...
      Bhojpuri:[`${n}, bas tohar yaad aa gail 😄 Ka haal ba?`, `${n}, chai aur baat pending ba. Kab milab? ☕`, `Pranam ${n}! Aaj muskura da, din mast jaai 😄`],
    ...
    Sorry:{
      ...
      Bhojpuri:[`${n}, maaf kari. Galti ho gail, dil se sorry 🙏`, `${n}, sorry. Agila chai hamar taraf se ☕`, `${n}, maaf kar da. Baat ke yahin khatam karal jaao.`],
...
function openHisaabSlip(id, isEditMode = false){
  ...
  const dueStr = k.dueDate ? escapeHtml(k.dueDate) : todayISO();

  // Mode: View mode by default, Edit mode only when user clicks '✏️ Edit Parchi'
  const modalHtml = `
    <div id="slip-modal-overlay" style="position:fixed; inset:0; background:rgba(0,0,0,0.72); z-index:99999; display:flex; align-items:center; justify-content:center; padding:14px;" onclick="if(event.target.id==='slip-modal-overlay') closeHisaabSlip()">
      <div style="background:#FFFDF8; border-radius:24px; width:100%; max-width:390px; max-height:92vh; overflow-y:auto; box-shadow:0 24px 50px rgba(0,0,0,0.35); border:2px solid #FFD4C4; animation:bbModalPop 0.2s ease-out; font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;">
        ...
        <div style="padding:18px 20px; background:#FFFDF8;">
          ...
          <!-- Top bar with Edit / Save toggle button -->
          <div style="display:flex; justify-content:space-between; align-items:center; font-size:0.8rem; color:#75615C; border-bottom:1px dashed #E5D5C5; padding-bottom:10px; margin-bottom:14px;">
            <span>Taarikh: <b>${todayISO()}</b></span>
            
            ${!isEditMode ? `
              <button type="button" onclick="openHisaabSlip('${k.id}', true)" style="background:#FFF0E6; border:1.5px solid #FFD4C4; color:#FF6B57; font-weight:800; font-size:0.78rem; padding:5px 12px; border-radius:8px; cursor:pointer; display:flex; align-items:center; gap:4px;">
                <span>✏️ Edit Parchi</span>
              </button>
            ` : `
              <button type="button" onclick="saveParchiEdits('${k.id}')" style="background:#247C32; border:none; color:#fff; font-weight:800; font-size:0.78rem; padding:6px 14px; border-radius:8px; cursor:pointer; display:flex; align-items:center; gap:4px; box-shadow:0 2px 6px rgba(36,124,50,0.3);">
                <span>✓ Done</span>
              </button>
            `}
          </div>
          ...
          <div style="margin-bottom:14px;">
            <div style="font-size:0.72rem; color:#75615C; text-transform:uppercase; font-weight:800; margin-bottom:4px;">Hisaab Banam:</div>
            ${!isEditMode ? `
              <div style="font-size:1.25rem; font-weight:900; color:#261818;">${escapeHtml(k.name)}</div>
              ${k.phone ? `<div style="font-size:0.82rem; color:#75615C; margin-top:2px;">📞 ${escapeHtml(k.phone)}</div>` : ''}
            ` : `
              <div style="display:grid; grid-template-columns: 1fr 1fr; gap:8px;">
                <input type="text" id="slip-edit-name" value="${escapeHtml(k.name)}" placeholder="Naam" style="width:100%; padding:8px 10px; border-radius:8px; border:1.5px solid #FFD4C4; font-size:0.9rem; font-weight:700; background:#fff; box-sizing:border-box;" />
                <input type="tel" id="slip-edit-phone" value="${escapeHtml(k.phone||'')}" placeholder="WhatsApp No." style="width:100%; padding:8px 10px; border-radius:8px; border:1.5px solid #FFD4C4; font-size:0.9rem; font-weight:700; background:#fff; box-sizing:border-box;" />
              </div>
            `}
          </div>

          <!-- Financial Breakdown -->
          <div style="background:#FFF5EC; border-radius:14px; padding:14px; margin-bottom:14px; border:1.5px solid #FFE2D1;">
            ...
            ${!isEditMode ? `
              ...
              <div style="border-top:1.5px dashed #FFD4C4; padding-top:8px; margin-top:6px; display:flex; justify-content:space-between; align-items:baseline;">
                <span style="font-size:1rem; font-weight:900; color:#261818;">Baaki Rashi (Due):</span>
                <span style="font-size:1.35rem; font-weight:900; color:#FF5E48;">${fmtMoney(due)}</span>
              </div>
            ` : `
              <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
                <span style="font-size:0.82rem; color:#5A302B; font-weight:700;">Kul Rashi (Total):</span>
                <div style="display:flex; align-items:center; gap:4px;">
                  <span style="font-weight:800; color:#5A302B;">₹</span>
                  <input type="number" id="slip-edit-total" value="${totalAmt || due}" oninput="recalcSlipModalDue()" style="width:90px; padding:6px 8px; border-radius:6px; border:1.5px solid #FFD4C4; font-weight:800; font-size:0.95rem; text-align:right;" />
                </div>
              </div>
              <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
                <span style="font-size:0.82rem; color:#247C32; font-weight:700;">Jama Hua (Paid):</span>
                <div style="display:flex; align-items:center; gap:4px;">
                  <span style="font-weight:800; color:#247C32;">- ₹</span>
                  <input type="number" id="slip-edit-paid" value="${paidAmt}" oninput="recalcSlipModalDue()" style="width:90px; padding:6px 8px; border-radius:6px; border:1.5px solid #C8E6C9; font-weight:800; font-size:0.95rem; text-align:right; color:#247C32;" />
                </div>
              </div>
              <div style="border-top:1.5px dashed #FFD4C4; padding-top:8px; margin-top:6px; display:flex; justify-content:space-between; align-items:center;">
                <span style="font-size:0.95rem; font-weight:900; color:#261818;">Baaki Rashi (Due):</span>
                <div style="font-size:1.25rem; font-weight:900; color:#FF5E48;" id="slip-edit-due-display">₹${due}</div>
              </div>
            `}
          </div>
          ...
          <div style="display:flex; justify-content:space-between; align-items:center; font-size:0.82rem; margin-bottom:16px;">
            <span style="color:#75615C;">Bhugtan Taarikh: <b>${dueStr}</b></span>
            <span style="background:${isPaid?'#DFF8DF':(isOverdue(k)?'#FFEAE8':'#FFF0C7')}; color:${isPaid?'#247C32':(isOverdue(k)?'#D93829':'#9A6810')}; font-weight:800; padding:4px 9px; border-radius:6px;">
              ${isPaid ? '✓ PAID' : (isOverdue(k)?'⚠️ OVERDUE':'⏳ PENDING')}
            </span>
          </div>
...
function recalcSlipModalDue(){
  const tot = Number(document.getElementById('slip-edit-total').value || 0);
  const paid = Number(document.getElementById('slip-edit-paid').value || 0);
  const due = Math.max(0, tot - paid);
  const dEl = document.getElementById('slip-edit-due-display');
  if(dEl) dEl.innerText = '₹' + due;
}

function saveParchiEdits(id){
  const k = state.khata.find(x => x.id === id);
  if(!k) return;
  const nameEl = document.getElementById('slip-edit-name');
  const phoneEl = document.getElementById('slip-edit-phone');
  const totEl = document.getElementById('slip-edit-total');
  const paidEl = document.getElementById('slip-edit-paid');
  const noteEl = document.getElementById('slip-edit-note');

  if(nameEl) k.name = nameEl.value.trim() || k.name;
  if(phoneEl) k.phone = phoneEl.value.trim();
  if(totEl) k.amount = Number(totEl.value || 0);
  if(paidEl) k.paidAmount = Number(paidEl.value || 0);
  if(noteEl) k.note = noteEl.value.trim();

  if(k.amount > 0 && k.paidAmount >= k.amount) k.status = 'paid';
  else if(k.paidAmount > 0) k.status = 'partial';
  else k.status = 'pending';

  persist();
  renderApp();
  showToast('Parchi update ho gayi! ✅');
  // Re-open in view mode with updated values
