...
function viewInvoiceForm(){
  ...
  <div class="invoice-form-card"><div class="invoice-grid"><div class="field-block"><label class="field-label">Bill number</label><input value="${escapeHtml(d.number)}" oninput="bbSetDraft('number',this.value)"></div><div class="field-block"><label class="field-label">Bill date</label><input type="date" value="${escapeHtml(d.date)}" oninput="bbSetDraft('date',this.value)"></div><div class="field-block"><label class="field-label">Due date (optional)</label><input type="date" value="${escapeHtml(d.dueDate)}" oninput="bbSetDraft('dueDate',this.value)"></div></div>
  <details class="invoice-details" open><summary>Aapka business</summary><div class="invoice-grid"><div class="field-block"><label class="field-label">Business/name</label><input placeholder="Shree Balaji Traders" value="${escapeHtml(d.seller)}" oninput="bbSetDraft('seller',this.value)"></div><div class="field-block"><label class="field-label">Phone</label><input inputmode="tel" value="${escapeHtml(d.sellerPhone)}" oninput="bbSetDraft('sellerPhone',this.value)"></div><div class="field-block"><label class="field-label">GSTIN (optional)</label><input maxlength="15" value="${escapeHtml(d.gstin)}" oninput="bbSetDraft('gstin',this.value.toUpperCase())"></div></div><div class="field-block"><label class="field-label">Address</label><textarea oninput="bbSetDraft('sellerAddress',this.value)">${escapeHtml(d.sellerAddress)}</textarea></div></details>
  <details class="invoice-details" open><summary>Customer</summary><div class="invoice-grid"><div class="field-block"><label class="field-label">Customer name</label><input placeholder="Customer" value="${escapeHtml(d.buyer)}" oninput="bbSetDraft('buyer',this.value)"></div><div class="field-block"><label class="field-label">Phone (optional)</label><input inputmode="tel" value="${escapeHtml(d.buyerPhone)}" oninput="bbSetDraft('buyerPhone',this.value)"></div></div><div class="field-block"><label class="field-label">Address (optional)</label><textarea oninput="bbSetDraft('buyerAddress',this.value)">${escapeHtml(d.buyerAddress)}</textarea></div></details>
  <div class="section-title">Items</div><div class="invoice-items">${d.items.map((i,n)=>`<div class="invoice-item"><div class="item-head"><b>Item ${n+1}</b>${d.items.length>1?`<button onclick="bbRemoveInvoiceItem(${n})">Remove</button>`:''}</div><input class="item-desc" placeholder="Product/service description" value="${escapeHtml(i.description)}" oninput="bbSetItem(${n},'description',this.value)"><div class="item-numbers"><label>Qty<input type="number" inputmode="decimal" min="0" step="0.01" value="${escapeHtml(i.qty)}" oninput="bbSetItem(${n},'qty',this.value)"></label><label>Rate ₹<input type="number" inputmode="decimal" min="0" step="0.01" value="${escapeHtml(i.rate)}" oninput="bbSetItem(${n},'rate',this.value)"></label></div></div>`).join('')}</div><button class="ghost-btn" onclick="bbAddInvoiceItem()">+ Item add karein</button>
  ...
  <div class="invoice-actions"><button class="primary-btn" onclick="bbSaveInvoice(false)">Save Bill</button><button class="ghost-btn" onclick="bbSaveInvoice(true)">Save & PDF/Print</button></div></div>`;
}
...
function bbInvoiceMenu(id){
  ...
  const action=prompt('Type: PRINT, DUPLICATE, or DELETE','PRINT');if(!action)return;
  if(action.toUpperCase()==='PRINT')bbPrintInvoice(x);
  else if(action.toUpperCase()==='DUPLICATE'){state.invoiceDraft={...JSON.parse(JSON.stringify(x)),id:'inv-'+Date.now().toString(36),number:bbInvoiceNumber(),date:todayISO(),createdAt:Date.now()};navigate('invoice-new');}
  else if(action.toUpperCase()==='DELETE'&&confirm('Ye bill delete karein?')){bbSaveInvoices(bbLoadInvoices().filter(v=>v.id!==id));renderApp();showToast('Bill delete hua');}
}
