...
function openInvoicePayment(id){
  ...
  el.innerHTML=`<div class="bb-sheet-backdrop" onclick="this.parentElement.remove()"></div><div class="bb-sheet payment-sheet"><div class="bb-sheet-handle"></div><h3>Payment · ${escapeHtml(inv.buyer)}</h3><div class="payment-due-line"><span>Pending</span><b>₹${bbMoney(info.due)}</b></div>${(inv.payments||[]).length?`<div class="payment-history">${inv.payments.map(p=>`<div><span>${escapeHtml(p.date)} · ${escapeHtml(p.mode)}</span><b>₹${bbMoney(p.amount)}</b><button onclick="bbDeleteInvoicePayment('${id}','${p.id}')">×</button></div>`).join('')}</div>`:''}${info.due>0?`<label>Amount received ₹<input id="invoice-payment-amount" type="number" inputmode="decimal" min="0.01" max="${info.due}" placeholder="Jitna mila" value=""></label><label>Date<input id="invoice-payment-date" type="date" value="${todayISO()}"></label><label>Mode<select id="invoice-payment-mode"><option>UPI</option><option>Cash</option><option>Bank</option><option>Card</option><option>Other</option></select></label><label>Reference/note<input id="invoice-payment-note" type="text" placeholder="UPI reference, cash..."></label><div class="payment-action-grid"><button class="ledger-save received" onclick="bbSaveInvoicePayment('${id}')">Save Partial/Payment</button><button class="ledger-save full-paid" onclick="bbMarkInvoiceFullPaid('${id}')">✓ Full Paid ₹${bbMoney(info.due)}</button></div>`:'<div class="paid-celebrate">✅ Bill poora paid hai</div>'}</div>`;document.body.appendChild(el);
}
function bbSaveInvoicePayment(id,forcedAmount){const list=bbLoadInvoices(),snapshot=JSON.stringify(list),inv=list.find(x=>x.id===id);if(!inv)return;const before=bbInvoicePaymentInfo(inv),amount=forcedAmount===undefined?Number(document.getElementById('invoice-payment-amount')?.value):Number(forcedAmount),date=document.getElementById('invoice-payment-date')?.value,mode=document.getElementById('invoice-payment-mode')?.value,note=document.getElementById('invoice-payment-note')?.value.trim();if(!amount||amount<=0||amount>before.due+.005){showToast('Pending amount tak valid payment daalein');return;}inv.payments=inv.payments||[];inv.payments.unshift({id:'pay-'+Date.now().toString(36),amount,date:date||todayISO(),mode:mode||'Other',note,createdAt:Date.now()});inv.updatedAt=Date.now();bbSaveInvoices(list);document.getElementById('invoice-payment-sheet')?.remove();bbTrack('invoice_payment_add',{mode:mode||'Other',cleared:amount>=before.due});renderApp();bbShowUndo(amount>=before.due?'Bill Full Paid mark hua':'Payment save hui',()=>{bbSaveInvoices(JSON.parse(snapshot));renderApp();});}
function bbMarkInvoiceFullPaid(id){const inv=bbLoadInvoices().find(x=>x.id===id);if(!inv)return;const due=bbInvoicePaymentInfo(inv).due;if(!due)return;if(!confirm(`Kya ₹${bbMoney(due)} poora payment sach mein mil gaya?`))return;bbSaveInvoicePayment(id,due);}
function bbShowUndo(message,undoFn){
  document.getElementById('bb-undo-toast')?.remove();const el=document.createElement('div');el.id='bb-undo-toast';el.className='bb-undo-toast';el.innerHTML=`<span>${escapeHtml(message)}</span><button>UNDO</button>`;document.body.appendChild(el);let active=true;const timer=setTimeout(()=>{active=false;el.remove();},6000);el.querySelector('button').onclick=()=>{if(!active)return;active=false;clearTimeout(timer);undoFn();el.remove();showToast('Change undo ho gaya ✅');};
}
